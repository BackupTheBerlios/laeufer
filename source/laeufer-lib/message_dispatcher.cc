...
bool message_dispatcher::notify( int file_handle, void F( void ) ){
  noti = new notifier(file_handle,F); // create a notifier
  noti->start(); //start the notifier
  return true;  //hmm, maybe we will need this later
}
