...
zum einen Daten, wie man Sie auch vom PKW her kennt, wie z.B. die
...
\item[Interaktion] Um die teilweise weit\-gehenden Ein\-stellungen des
...
bezieht, natürlich von besonderem Interesse ist.  Ausserdem sind sie
kommerziell in großen Stückzahlen verfügbar, was den Preis pro PDA
stetig sinken läßt.
...
soll aber gerade auch bei der Auswahl des PDAs auch der spätere Nutzer
des Läufers und seine Wünsche Berücksichtigung finden. Dieser erwartet
von einem solchen Gerät einen Mehrfachnutzen.  Es soll also sowohl als
,,Bordcomputer'' des Läufers fungieren können, aber gleichzeitig ein
...
\item[GUI-Flexibilität] Das Projekt Läufer geht in vielen Bereichen
  einen Design orientierten Weg, der nicht unerheblich zu dem großen
...
\item[Dokumentation und Support] Da die Entwicklung der Software über
  ...
  keine Entwicklungstools mehr erwerben kann.
...
ausser Acht läßt, nach den Betriebssystemen PalmOS, WindowsCE und
...
Weiterentwicklung mittlerweile eingestellt wurde.  Ausserdem paßt
...
\item[PalmOS] Dieses System wurde von der Firma 3Com entwickelt und
  ...
  guten und frei verfügbaren Dokumentation der Programmierung für
  diese Geräte geführt.
...
\item[Windows CE / Pocket PC] PDAs mit diesem System von Microsoft
  ...
  Dokumentation und Support der Programmierung sind wie für Microsoft
  typisch gut, die Entwicklungsumgebung ist kostenlos erhältlich.
  Allerdings ist WindowsCE genauso wie PalmOS ein reines PDA-System,
  so daß die Entwicklung für diese PDAs mit diesen speziellen Tools
  erfolgen muß und zum Testen der Software ein Emulator zum Einsatz
  kommt.
...
\item[Linux] Linux ist im gesamten PDA und Embedded-Bereich ein
  ...
  Diese Gleichheit in der Software bringt Linux auf dem PDA eine
  große Flexibilität, da direkt die gesamte Software, die es für
  Linux gibt, verfügbar ist.  Dies betrifft insbesondere die
  Bibliotheken für Entwickler.  So existieren z.B. mehrere
  GUI-Bibliotheken, die noch dazu auf verschiedenen Wegen mit der
  Hardware kommunizieren können.  Diese Auswahl an Möglichkeiten ist
  für unsere Anwendung geradezu ideal.
...
über Multitasking verfügt.  Ausserdem ist es im Bereich der GUI sehr
...
\subsection{Anforderungen}
Für den IPAQ stehen mehrere Linux-Distributionen zur Auswahl.  Es
mußte sich also innerhalb des Projektes für eine entschieden werden.
Dabei waren die Anforderungen der Entwickler mechatronischer
Komponenten ebenso zu berücksichtigen wie die späterer Nutzer des
Läufers.
...
installierten Software einfach installieren zu können.  Dies gilt zum
einen natürlich und insbesondere für die Systemsoftware, also die
Distribution an sich, aber auch für die für den Läufer speziell
angefertigte Software.



\subsection{Die Alternativen}\label{mw_ipaq_linux}
Compaq selbst unterstützt maßgeblich die Portierung des
Linux-Kernels, ist aber bei der darauf aufsetzenden Linux-Distribution
mittlerweile aus der Entwicklung einer eigenen ausgestiegen, da
mehrere aus der Sicht Compaqs besser Alternativen zur Verfügung
stehen.  Derzeit befindet sich Linux auf PDAs und insbesondere auf dem
IPAQ unter dem Einfluß starker Entwicklertätigkeit, so daß viele
Entwicklungen sich noch im Zustand ,,Projekt'' befinden und erst nach
und nach zu ,,Produkten'' werden.  Das prominenteste ,,Produkt''
dürfte der Sharp Zaurus sein, dessen Software weitgehend mit der
freien Oberfläche OPIE übereinstimmt.
...
\item[PocketLinux] PocketLinux\cite{plinux} kann man wohl mit Fug und
  Recht als das ambitionierteste der Projekte bezeichnen.  PocketLinux
...
\item[Familiar Linux] Familiar\cite{flinux} ist die Brot-Und-Butter
  ...
  Probleme mit Abhängigkeiten zwischen Sodtwarepaketen.  So führt
  ...
  waren.  Ebenfalls verfügbar sind die ersten X11-basierte Anwendungen,
  die speziell für Familiar entwickelt wurden.  Diese Anwendungen
  berücksichtigen decken den Bereich der Organizer-Software ab und
  berücksichtigen natürlich die spezielle Hardware, auf der sie
  laufen.
...
\item[Intimate] Das Ziel dieses Projektes ist es, ein komplettes
  ...
  wird dies ein IBM Microdrive sein, das 1GB an Speicher im Formfaktor
...
\item[QTOPIA] QTopia ist keine eigene Distribution, sondern ein Set
  ...
  QT genießt ausserdem den Ruf einer sehr guten Dokumentation, was
  ...
  Produktreife von QTOPIA spricht.
\end{description}
...
Komponenten für den Läufer arbeiten werden. Ausserdem war zu Beginn
...
Es ist also nötig, die Entwickler so gut es möglich ist voneinander
...
voneinander zu trennen, ist eine Kommunikationsstruktur nötig, die dem
...
\item[Treiber] Diese Komponenten stellen ein Interface zu den
  ...
  zum CANBus zur Verfügung zu stellen.  Ausserdem müssen diese Treiber
...
Aufgabe war es also, zwei Schnittstellen zu definieren. Im einzelnen
...
Die GUI muß an die Treiber Befehle geben können.  Dies läßt sich
...
\item[Signals und Slots] Da für die GUI das Produkt ,,QTopia'' der
  ...
  zu den anderen Möglichkeiten nicht zu den Standard-Techniken gehört,
  soll hier eine Erläuterung dieses Verfahrens gegeben werden.
...
\end{verbatim}
  ...
  Aufgrund dieser Flexibilität wurde sich für diese Art der Anbindung
  entschieden.  Informationen über QT finden sich auf \cite{mwtt}.
  Der einzige Nachteil an dieser Lösung ist die Abhängigkeit von QT,
  was bei der Nutzung von QT als GUI für den PDA aber kein großes
  Hindernis darstellt.
...
Verteilermechanismus würde diese Nachricht dann als Signal an den
entsprechenden Treiber als Signal gesendet.
...
entwickelt zu haben. Da dieser Entwickler ja weiß, welche ID sein
Gerät hat und welche Operationen es versteht, kann er diese manuell
eingeben und die korrekte Funktionsweise entweder anhand von
Änderungen am Gerät selber (z.B.: Lampe blinkt) oder aber anhand von
Rückmeldung über den CAN-Bus beurteilen.  Genau dies soll der
DebugTreiber ermöglichen: Manuelles Senden und Empfangen von
Nachrichten an ein bzw. von einem bestimmten Gerät, um den
Hardware-Entwicklern die Arbeit zu erleichtern.
...
hier auswählen, einen sogenannten ,,Broadcast'' zu senden, der dann an
