
\overlays{2}{%
  \begin{slide}{Das Fahrerinterface}
      \begin{minipage}{7.0cm}
        \untilSlide*{1}{%
          Gründe für einen PDA:
          \begin{itemize}
          \item Genügend Rechenleistung (Hochsprachen)
          \item Display (Warnhinweise)
          \item Interaktion (Einstellungen, Routing, etc.)
          \end{itemize}
        }
      \fromSlide*{2}{%
        Gründe für \emph{diesen} PDA
        \begin{itemize}
          \item Display
          \item Rechenleistung
          \item Linux (Multitasking, Programmierung)
        \end{itemize}
      }
    \end{minipage}
    \begin{minipage}{3cm}
      \begin{center}
        \includegraphics[width=3cm]{zaurus.eps}
      \end{center}
    \end{minipage}
  \end{slide}
}


% ============================================================


\overlays{2}{
  \begin{slide}{DPI aus Entwicklersicht}
    \onlySlide*{1}{
      \begin{center}
        Informal: Kommunizierende Automaten\\
        \includegraphics[width=9cm, angle=-90]{zustaende.eps}
      \end{center}
    }
    \onlySlide*{2}{
      Vaterklasse \texttt{Driver} für alle Treiber
      \begin{itemize}
      \item GUI ruft \texttt{Blinken()} des Treibers auf.
      \item \texttt{send(Message)}: Versenden von \emph{Nachrichten}.
      \item \texttt{receive(Message)}: Empfang von Nachrichten.
      \end{itemize}
    }

  \end{slide}
}


% ============================================================


\begin{slide}{Weg der Nachricht durch das DPI}
  \begin{itemize}
  \item \texttt{Driver.send()} überprüft die Nachricht
  \item \texttt{message\_dispatcher.send()} sendet mit richtigem
    \texttt{bus\_manager} heraus 
  \item \texttt{bus\_manager.send()} sendet auf passendem \texttt{bus}
  \end{itemize}
  $\to$ Zweifache Indirektion: Mehrere logische und physische Busse.
\end{slide}