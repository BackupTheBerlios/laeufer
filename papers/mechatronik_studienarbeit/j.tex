...
und ein Multi-Master-Protokoll, das sich auf CAN implementieren läßt.
	...
	bemerkt würde) somit, für die Aufsummierung der Wahrscheinlichkeiten des Verlustes an der $x$ten Stelle
	$$p_n = p^{16n + 15} * (1-p)$$
	und im Limes
	\begin{equation}
	  \begin{split}
	    \lim_{n \rightarrow \inf} \Sigma_{k = 0}^n p_k &= (1-p) * p^{15} * \lim_{n \rightarrow \inf} \Sigma_{k=0}^n p^{16n}\\
	    & = 1-p * p^15 * \lim_{n \rightarrow \inf} \frac{p^{(n+1)*16} - 1}{p^{16} - 1}\\
...
eine sehr spezifische Semantik erzwingt. Zudem implementiert es den letzten noch fehlenden Aspekt des Korrektheitskriteriums.
	...
	(bei LLO werden aufgrund der maximal 30 verwendeten nicht-Controller-Adressen also höchstens 30 Instanzen des Protokolls zugleich ausgeführt).
	...
	Treiber seitens des PDAs\footnote{welche ihre Klienten üblicherweise auf einem bestimmten Kommunikationskanal erwarten} können mittels
	...
	Zeitpunkt $t$ ein Tupel $(b, t)$ gespeichert, wobei eine (partielle) Zuordnung $sent b \mapsto t$ resultiert,
