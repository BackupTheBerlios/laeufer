...
Die Mechatronik im Projekt L"aufer muß einer Reihe projektspezifischer
...
f"ur die Zukunft nicht abschätzen l"asst, und neue Komponenten
jederzeit einfach einzugliedern sein sollen muss das System leicht
erweiterbar sein. Für die Zielgruppe zur Nutzung der Platinen muss das
System mit vertretbarem Aufwandt bedienbar sein. Für den nutzenden
Ingenieur ist der Effekt und nicht der Weg interessant.  Die
begrenzten Resourcen eines autnomen Systems wie dem L"aufer erfordern
...
als auch in der Implementierung m"ussen m"ogliche Extreme in der
Nutzung des Fahrzeugs ber"ucksichtigt werden. Dies beinhaltet auch
eine gewisse Fehlertoleranz.  Nicht zuletzt muss die Lösung zum einen
durch das vorhandene Know How im Umfeld des Projekt L"aufer umsetzbar
als auch finanzierbar sein.  Bei der Zusammenstellung der Platine muss
ausserdem die Verf"ugbarkeit der Bauelemente sichergestellt sein.  Wie
...
begleitete die Firma Cetron den gesammten Prozess der
Platinenentwicklung. Von der Idee, dem Prototypen, der Auswahl der
Komponenten, des Microcontrollers, dem Layout der Platine in den zwei
...
ausserdem selbst durch diverse Schutzmaßnahmen vor Überhitzung
und/oder Stromspitzen geschützt.

\subsection{Design}
Um sich in das Design des Läufers zu integrieren sind sowohl die
Erweiterbarkeit als auch die simple Grösse der Platine von
...
also etwa die Grösse einer Checkkarte (siehe Abbildung
\ref{Platine-Karte}).
...
Switches waren nicht integriert und die andeschlossene Peripherie
...
weiterer Ideen zur Anäherung an das Ziel einer multifunktionalen
...
ständiger Cooperation mit den Maschinenbaustudenten des Projekts und
...
Entwicklungsumgebungen, Energieverbrauch, Grösse des Programm- und
...
\item Infineons SABC515-Familie bietet mit zum Teil integrierten
  ...
  Microcontroller sind die Microcontroller für die vorgesehene
  Anwendungen schnell genug. Der Preis von 9 bis 14 \geneuronarrow{}
  pro Microcontroller ist vergleichbar hoch. Desweiteren müssen für
  den SABC515 die Programme in externen Speicherbausteinen abgelegt
  werden, die unnötig viel Platz in Anspruch nehmen.
  
\item Die Microcontrollerreihe 68HC11 von Motorola wird bereits
  vielfach inder Autoindustrie genutzt.  Die Betrachtungen beschränken
  ...
  Beispielapplikationen erhältlich. Die betrachteten Microcontroller
  ...
  kein Stromsparmodus.  auch die Preise von bis zu 14 \geneuronarrow{}
  sprechen gegen den 68HC11x.
  
\item Microchip ist in den USA für die PIC-Microcontroller bekannt.
  ...
  schränkt die Eignung für die Mechatrinikplatine jedoch wieder ein.
...
\item Mit dem SX52 von Ubicom steht noch ein weiterer
  ...
  Preis ist mit ca. 7 \geneuronarrow{} besser als der des Motorolas
  und gleichauf mit dem PIC-Controller.  Durch die
  PIC-Kompa\-tibilität stehen auch für den SX52 viele Resourcen offen.
...
in simulierter Umgebung getestet werden muss, sondern sofort im
...
Laufzeit an- und ausschltbar ist. Der Microcontroller hat eine
...
löst einen Interrupt aus bzw. ,,weckt'' den Mocrocontroller aus dem
Stromsparmodus.
...
werden die genutzen Bauelemente und Funktionalitäten der Platine
beschrieben.
...
\item[Serial Peripheral Interface (SPI)]\label{cr_SPI-Bus} Die große
  ...
  Microprozessor nötig.  Zu diesem Zweck sind der CAN-Bus-Chip, zwei
  ...
  Lösung spart nicht nur wertvolle Pins am Port des Microcontrollers,
  sie garaniert auch das keine zwei oder mehr Geräte auf den
...
\item[Kommunikation] Neben dem SPI-Bus zur internen Kommunikation
  ...
  bereits im vorangegangenem Kapitel\ref{kapitel_jameson} beschrieben,
  ...
  an den interruptfähigen Port des Microcontrollers ist eine
...
\item[Analog-Digital-Wandler] Der LTC 1594 von Linear Tchnologie
  ...
  Microcontroller angeschlossen. Zur Konfigurations werden anfangs
  ...
  Über Jumper lässt sich eine Rückkopplung der Brückenausgänge mit den
  Eingängen des Anlog-Digital-Wandlers herstellen.  Der Strom an der
...
\item[Digitale Eingänge] Mit dem ULN2004A gibt es über 7 Anschlüsse
  ...
  Schlatungen enthaltende Baustein von STMicroelectronics Akzeptiert
  eine Eingangsspannung von fünf bis zu acht Volt, je nach
  Eingangsstrom.  Drei der Eingänge werden an den interruptfähigen
  Port des Microcontrollers durchgeschaltet. Dadurch ergibt sich eine
  weitere Möglichkeit die Platine von ausserhalb zu ,,wecken''.
...
\item[Speicher] Mit dem FM24C64 steht ein 64 Kilobyte großer seriell
  ...
  Der FM24C64 ist über ein sogennates ,,Two-wire
  Interface''\footnote{Two-wire. engl: zwei Drähte} an den SX
  angeschlossen. Dabei generiert der SX als Master das Tacktsignal für
...
Assembler-Programme des Microcontrollers beschrieben.
...
\item Der Programmspeicher muss möglichst effizient von den Modulen
  genutzt werden.
...
\subsection{Sicherheit}
Die Implementierung der Platinen muss eventuelle Fehlfunktionen
berücksichtigen und adäquat behandeln ohne die Kommunikationsebene zu
stören.  Desweiteren muss es einen sicheren Zustand geben in den die
Platine bei etwaiger Fehlfunktion versetzt wird um Kurzschluss,
...
in Kontakt mit der Steuereinheit treten, auf der Platine muss der
...
und nicht zuletzt muss der Microcontroller die zeitkritische serielle
...
zu können, muss die Programmierung der Komponnenten einer klaren
Konvention folgen.So können auch neue Implemetierungen diesem Muster
...
der erhältichen Entwicklungsumgebung der Firma Parallax zusammen.  Das
...
beziehungsweise die Debugroutinen anzuwenden, benötigt man ausserdem
...
umrandet. Auf diese Art lässt sich schnell feststellen ob das Programm
...
grob umrissen, um eine Verständniss der folgenden Erklärungen zu
ermöglichen.  Ich gehe davon aus, das Begriffe wie Akkumulator, Clock
(oder auch Zeitgeber) und Register bereits bekannt sind.

Die Programme des SX52 sind in einem 64 Kilobyte grossem
...
Argumente. Es handelt sich beim SX52 um eine RISC-Archutektur, das
...
\item[Rollover-Interrupt] Der \footnote{engl.roll over: überschlagen,
  ...
  oder RETI). Diese Befehle sorgen nun dafür, dass der Programmzähler,
...
\item[Multi-Input-Wakeup-Interrupt] Der Multi-Input-Wakeup-Interrupt
  ...
  ,,aufwachen'' des Microcontrollers, wenn dieser im SLEEP-Modus war,
...
Bit im FSR wird dabei immer auf Null gesetzt. Um jetz trotzdem die
ungeraden Registerbänke adressieren zu können muss der Programmierer
...
Adressraum von 256 Byte ansprechen (Argumentlänge: neun Bit), drei der
...
gerader Wert in den ersten vier Bit steht.  Steht alo im Bit Nummer
...
Dirktive wird nicht zur Laufzeit, sondern bereits vor der
...
MACRO-Direktive. Programmcode, der in ein Macro eingebunden wird lässt
sich leicht aus dem Programm entfernen und wieder einsetzen. Diese
Eigenschaft ermöglicht es Programmteile genau zu platzieren und so
...
Macro-Direktiven die oben genannten Schwierigkeiten umgeht.
...
\item[CALL-Segment]\label{cr_call_seg} Das Call-Segment dient dazu die
  256-Byte Grenze des CALL-Befehls zu umgehen. Dieses Segment muss
  ...
  wenn dem CALL ein @ vorangestellt wird, so dass auch dies
...
\item[DATA-Segment] Wie bereits oben beschrieben stehen dem
  ...
  Macros ermöglichen es bei der Programmierung auf
  Registeradressierung zu verzichten.  Um das zu erreichen definiert
  man ein Macro in dem Platz für maximal 16 Register reseviert wird.
  Man benutzt bei der Programmierung dann nur noch die Variablen für
  den reservierten Platz.
  
  Um dann die entsprechende Bank zu reservieren, wird das Macro durch
  die ORG-Direktive an der entsprechenden Stelle eingefügt. Die erste
  Instruktion im Macro weist dann einer Variablen die aktuelle Adresse
  ...
  ohne schon bei der Implemetierung wissen zu müssen welche Bank er
  eigentlich nutzt.  Beipiel
\begin{verbatim}
MACRO     FOO   ; Macro namens FOO
...
\item[INTERRUPT-Segment] Benötigt man für ein Modul auch Routinen die
  ...
  nötigen Code in ein Macro.  Dieses Macro wird als Interrupt-Segment
  ...
  kein Interrupt-Rücksprungbefehl (RETI oder RETIW) stehen. Der muss
  am Ende der Macroliste stehen um sicherzustellen das alle Segmente
  ausgeführt werden.

\item[INIT-Segment] Jedes Modul benötigt auch einen Bereich im
  Initialbereich des Gesamtprogramms zur Initalisierung seiner
  Variablen. Dazu definiert man zu jedem Modul ein INIT-Macro.
\end{description}

Zusammengefasst könnte ein Programm nach den oben beschriebenen
...
\item[Serielle Schnittstelle] Die serielle Datenübertragung ist in der
  ...
  Fall nicht statt.  Es handelt sich bei der RS232 um eine asysnchrone
  ...
  Signal mit einer mehrfachen Taktferquenz abtasten und so das
  ...
  nächsten Interrupt, so lässt sich die Interrupt-Frequenz regulieren.
  Der Befehl RETIW macht das möglich. RETIW setzt den Interruptzähler
  auf den im Akkumulator W übergebenen Wert.  So lässt sich mit 108
  Zyklen pro Interrupt ein Takt von ungefär 115.200 Baud realisieren
  ...
  mehrere Module mit Interrupt-Segmenten genutzt werden, so muss die
...
deutet darauf hin das dei Interruptroutine nicht zu groß ausfallen
sollte, da sonst die Ausführung sonstiger Programme zu kurz kommt.
Nutzt man also das Modul, so muss zuvor eine Baudrate für die serielle
...
\item[Puls Weit Modulation]\label{cr_puls}Mittels Puls-Weit-Modulation
  läßt sich an digital angeteuerten geräten analogen Verhalten
  erzeugen.  Dabei wird die Trägheit der jeweiligen
  elektromechanischen Gerätes augenutzt um die digitalen Impulse zu
  ...
  Grösse $t_{i}$ bestimmt dann die Intensität mit der das am Ausgang
  angeschlossene Gerät betrieben wird.  Im folgenden wird $t_{i}$ auch
  Pulsweite genannt.  Ein angeschlossener Gleichstromotor zum Beispiel
  ...
  Interrups. In jedem Interrupt wird ein Zähler iterriert bis dieser
  ...
  einer Variable verglichen die dei Pulsweite bestimmt. Je nach
  Anwendung kann man nun die Pulsweite veändern, indem man sie
  iterriert oder decrementiert bzw. unterschiedlich initialisiert.
%reverse iterations
\begin{figure}
...
\item[SPI] Um eines der Geräte über den SPI-Bus zu adressieren muss
...
\item{ADC} Noch bevor der Analog-Digital-Wandler per Chip-Select
  aktiviert wird, werden 4 bit zum auswählen des zu wandelden Eingangs
...
\newpage
\section{Fazit}
Die Wahl des Microcontrollers hat sich als gut erwiesen, da sich eine
...
C-Kompiler verbinden, könnte man eine komplette und komfortable
