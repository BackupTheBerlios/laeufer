...
Spannungsspitzen zur Betriebszeit, Ausfallsicherheit zur Betriebszeit
und Sicherheit der Komponenten vor Fehlbedienung durch den Entwickler.
...
Näheres dazu steht in der Beschreibung der einzelnen Komponenten in Kapitel \ref{cr_peripherie} Periepherie.
...
Um von Ausfallsicherheit zur Betriebszeit auszugehen wäre ansich eine groß angelegte Testreihe im Läufer
unter verschiedenen Testbedingungen von Nöten. Die bisherigen Versuche und Tests außerhalb des Läufers 
waren auch erfolgreich. Ob es im Läufer dann noch ,,Überraschungen'' gibt läßt sich jetzt noch nicht sagen.
Wie sich die mechatronischen Applikationen der Maschinenbauer im Fall einer Störung verhalten 
...
vierlagige scheckkartengroße Trägerplatine ersetzt. Erst die
Entwicklung dieser zweiten Platine wurde von dieser Studienarbeit
begleitet. Der zweite Revision berücksichtigte dann eine Reihe
...
\item Mit dem SX52 von Ubicom steht noch ein weiterer
  PIC-Mikrocontroller zur Auswahl. Mit acht interruptfähigen PIN's ist
  ...
  spiegelt dabei die Arbeitsspeicherbelegung.  Näheres dazu im Kapitel
  \ref{Umgebung} Programmierumgebung.
...
Mikrocontroller hat eine Interrupt-funktion die ebenfalls zur Laufzeit
...
In dieser Applikation wird das jeweilige Chip-Select über einen
  Multiplexer gesetzt. Diese Lösung spart nicht nur wertvolle Pins am
  Port des Mikrocontroller, sie garantiert auch, das keine zwei oder
  mehr Geräte auf den Datentransfer reagieren. Dies macht die
...
\item[Kommunikation] Neben dem SPI-Bus zur internen Kommunikation
  ...
  beschrieben, ein CAN-Bus für die externe Kommunikation zur
...
\item[Digital-Analog-Wandler] Der am SPI-Bus angeschlossene
  Digital-Analog-Wandler LTC1446 von Linear Technology dekodiert eine $2\times12$ Bit lange
...
\item[Analog-Digital-Wandler] Der LTC 1594 von Linear Technology
...
\item[Low-Side-Switch]Mit dem Siemens Smart Octal Low Side Switch TLE
  ...
  Puls Weit Modulation in \ref{cr_puls}). Die notwendigen Signale für
...
\item[Brücken] Für verbrauchsstarke Gleichstromgeräte sind zwei
  ...
  Die aus zwei Bausteinen resultierenden 4 Anschlüsse, können für vier
...
Programmspeicher ohnehin eine geringe Verschachtelungstiefe der
...
erhältlichen Entwicklungsumgebung der Firma Parallax zusammen.  Die
Entwicklungsumgebung besteht aus der SX-Software SX.exe und dem
SX-Key \ref{cr_sx_key}.
...
indirekt adressierbare Bänke (siehe Abbildung \ref{cr_register}).
Jede Bank enthält 16 Register. Für die
Adressierung der globalen Register sind keine Vorkehrungen nötig.  Um
ein solches Register zu adressieren schreibt man einfach die absolute
Adresse des Registers (direkte Adressierung). Um die 16 ,,normalen''
Registerbänke zu referenzieren, muß zuvor die entsprechende
Registerbank über das FSR angegeben werden. Erst dann kann man unter
16 Registern der entsprechenden Bank wählen. Das heißt, das nur die
unteren vier Bit von Relevanz sind. Um eine solche (indirekte)
Registeradressierung anzukündiger müssen die ersten vier Bit (den
MSB\footnote{Most Significent Bits: höchstwertige Bits}MSB eine 1
darstellen.
...
nutzbar sind, zuletzt ver bleiben noch sechs ,,freie'' Register.
...
\item[DATA-Segment] Wie bereits oben beschrieben stehen dem
  ...
  zwangsweise benannt,so daß beim programmieren, wie bereits gesagt,
...
\item[Serielle Schnittstelle]\label{crRS232} Die serielle
  Datenübertragung ist in der Norm RS232 festgelegt.  Danach können
  zwei Computer über die Datenleitungen TxD (Transmit Data) und RxD
  ...
  voorangestelltes Startbit und ein nachgestelltes Stopbit
  ...
  Zyklen pro Interrupt ein Takt von ungefähr $115.200$ Baud
...
ausreichend Zeit zur Ausführung bekommen.
...
\item[Puls Weit Modulation]\label{cr_puls}Mittels Puls-Weit-Modulation
  ...
  nach $high$ übergeht (siehe dazu Abbildung \ref{cr_pwm}).
...
\begin{figure}
  ...
  \caption{Puls Weit Modulation einer linearen Funktion. Die fallende Flanke des PWM-Signals markiert jeweils das Ende eines Intervalls $z_i$.
...
Dokumentation wünschenswert.  Während meiner Recherchen für
Beispielprogramme stieß ich auch auf einen C-Compiler für
PIC-kompatible Mikrocontroller.  Würde man nun eine grafische
Oberfläche zur Nutzung der Module mit dem C-Compiler verbinden, könnte
man eine komplette und komfortable Mechatronik-Entwicklungsumgebung
als Produkt anbieten. Insbesondere im Bildungsbereich könnte dies
Anklang finden.
%Anleitung
%C-Compiler
%Techn Docu
