...
Prozessoren diese Informationen auswerten und Aktoren gezielt genutzt werden,
um z.B. Kräfte oder Bewegungen zu erzeugen, die auf das System und seine 
Umgebung zurückwirken.

Vor diesem Hintergrund steht den Ingenieuren im Projekt mit der Platine
...
\item Infineons SABC500-Familie bietet mit zum Teil integrierten
  ...
  Microcontroller für die vorgesehene
...
\item Die Microcontrollerreihen 68HC11, 68HC05, 68HC08 von Motorola wird bereits
...
\item Mit dem SX52 von Ubicom steht zusätzlich ein 
  PIC-kompatibler Microcontroller zur Auswahl. Mit acht interruptfähigen PIN's ist
...
Microcontroller hat eine Interruptfunktion, die ebenfalls zur Laufzeit
...
Microcontroller aus dem Stromsparmodus.
...
\item[Kommunikation] Neben dem SPI-Bus zur internen Kommunikation
  ...
  Clear To Send) an den interruptfähigen Port des Microcontrollers ist
...
\item[Low-Side-Switch]Mit dem Siemens Smart Octal Low Side Switch TLE
  ...
  die PWM gehen direkt vom Microcontroller aus (also ohne den Umweg
...
In dieser Applikation wird das jeweilige Chip-Select über einen
  ...
  Port des Microcontroller, sie garantiert auch, das nur genau ein Gerät
...
\item[LED] Um ein optisches Feedback (zum Beispiel für den Fall einer
  ...
  LEDs\footnote{LED = Light Emitting Diode} an den Microcontroller
...
den Microcontroller erfolgt über eigens dafür vorgesehene Anschlußpins
auf der Platine.
...
\item[SLEEP-Befehl] Durch den SLEEP-Befehl wird die Clock
  ausgeschaltet und der Microcontroller nur
...
\item[Endlosschleife] Man kann das Erreichen der höchsten
...
\item[Speicher] Der Speicher ist über eigene Anschlüsse an den Microcontroller angeschlossen.
Über eine serielle Datenleitung erfolgt der Datentransfer. Für jede Operation (ob schreiben oder lesen) , wird
  ...
  Adressierung sendet der Master (hier der Microcontroller) beliebig viele acht-Bit Worte und
...
% Die Wahl des Microcontrollers hat sich als richtig erwiesen, da sich
